...
export class IntegrationsController {
  ...
  @Post('playwright/run')
  async createPlaywrightRun(@Body() createTestRunDto: CreateTestRunDto, @Request() req: any) {
    // Validate organizationId matches the authenticated user/API key's organization
    const userOrgId = req.user?.organization?.id || req.apiKey?.organization?.id;
    
    if (!userOrgId) {
      throw new ForbiddenException('No organization associated with your account');
    }

    if (createTestRunDto.organizationId !== userOrgId) {
      throw new ForbiddenException('You can only create test runs for your own organization');
    }

    const testRun = await this.testRunsService.create(createTestRunDto, req.user?.id);
    this.testRunsGateway.notifyTestRunCreated(testRun);
    return { testRunId: testRun.id };
  }

  @Post('playwright/artifact')
  async addArtifact(@Body() createArtifactDto: CreateArtifactDto, @Request() req: any) {
