...
export class TestRunsController {
  ...
  async create(@Body() createTestRunDto: CreateTestRunDto, @CurrentUser() user: User) {
    const testRun = await this.testRunsService.create(createTestRunDto, user.id);
  ...
  @ApiResponse({ status: 200, description: 'List of test runs retrieved successfully' })
