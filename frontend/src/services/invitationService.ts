...
export const invitationService = {
  async create(data: CreateInvitationDto): Promise<InvitationWithUrl> {
    const response = await apiClient.post<InvitationWithUrl>('/invitations', data);
  ...
  async verify(token: string): Promise<InvitationVerification> {
    const response = await apiClient.get<InvitationVerification>(`/invitations/verify/${token}`);
    return response.data;
  },

  async accept(data: AcceptInvitationDto): Promise<{ message: string; userId: string; username: string }> {
